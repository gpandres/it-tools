...
function isIOC(value: unknown): value is IOC {
  ...
  return text(ioc.id, 128) && IOC_TYPES.includes(ioc.type as typeof IOC_TYPES[number]) &&
    text(ioc.value, 4096) && IOC_TAGS.includes(ioc.tag as typeof IOC_TAGS[number]) &&
...
function isTimelineEvent(value: unknown): value is TimelineEvent {
  ...
  return text(event.id, 128) && text(event.timestamp, 128) && text(event.description, 10000) &&
...
export function parseInvestigationCase(value: unknown): InvestigationCase | null {
  ...
  if (!text(item.id, 128) || !text(item.title, 500) || !text(item.description, 5000) || !text(item.findings, 50000)) return null;
