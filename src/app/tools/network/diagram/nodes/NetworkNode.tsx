...
export default function NetworkNode({ data, selected }: any) {
  ...
  return (
    <div className={`relative flex flex-col items-center justify-center p-3 rounded-xl border-2 backdrop-blur-md shadow-2xl transition-all ${colorClass} ${selected ? 'ring-2 ring-white shadow-white/20' : ''} min-w-[120px]`}>
      ...
      <Handle type="source" position={Position.Top} className="w-3 h-3 bg-zinc-400 border-2 border-black" id="top" />
      ...
      <Handle type="source" position={Position.Right} className="w-3 h-3 bg-zinc-400 border-2 border-black" id="right" />
      <Handle type="target" position={Position.Right} className="w-3 h-3 bg-zinc-400 border-2 border-black" />
      
      <Handle type="source" position={Position.Bottom} className="w-3 h-3 bg-zinc-400 border-2 border-black" id="bottom" />
      <Handle type="target" position={Position.Bottom} className="w-3 h-3 bg-zinc-400 border-2 border-black" />

      <Handle type="source" position={Position.Left} className="w-3 h-3 bg-zinc-400 border-2 border-black" id="left" />
      <Handle type="target" position={Position.Left} className="w-3 h-3 bg-zinc-400 border-2 border-black" />
    </div>
  );
}
