    ...
    <aside className={`${mobileOpen ? 'flex' : 'hidden'} absolute inset-x-0 bottom-0 z-40 h-[min(78dvh,42rem)] max-h-[78dvh] w-full flex-col overflow-hidden rounded-t-xl border-t border-[#1a1a1a] bg-[#050505] shadow-2xl md:relative md:flex md:inset-auto md:z-10 md:h-full md:max-h-none md:w-[19rem] md:rounded-none md:border-r md:border-t-0 md:shadow-none`} aria-label="Diagram toolbox">
      ...
      <div ref={scrollAreaRef} className="flex min-h-0 flex-1 flex-col overflow-y-auto p-3">
        ...
        <details className="border-b border-[#1a1a1a] py-4">
          <summary className="flex cursor-pointer items-center text-[10px] font-bold uppercase tracking-widest text-zinc-500">Templates<span className="ml-auto text-[9px] font-normal normal-case tracking-normal text-zinc-700">4 starter topologies</span></summary>
          <div className="mt-3 grid grid-cols-2 gap-2"><Button onClick={() => loadTemplate('Small Office')} variant="outline" size="sm" className="bg-black text-[10px]">Small Office</Button><Button onClick={() => loadTemplate('Enterprise Core')} variant="outline" size="sm" className="bg-black text-[10px]">Enterprise Core</Button><Button onClick={() => loadTemplate('DMZ')} variant="outline" size="sm" className="bg-black text-[10px]">DMZ</Button><Button onClick={() => loadTemplate('VLAN Segmentation')} variant="outline" size="sm" className="bg-black text-[10px]">VLAN Segmentation</Button></div>
        </details>

        <details className="border-b border-[#1a1a1a] py-4">
          <summary className="flex cursor-pointer items-center text-[10px] font-bold uppercase tracking-widest text-zinc-500">Validation<span className={`ml-auto text-[9px] font-normal normal-case tracking-normal ${validationIssues.length === 0 ? 'text-[#72e6b4]' : 'text-amber-300'}`}>{validationIssues.length === 0 ? 'no issues' : `${validationIssues.length} issue${validationIssues.length === 1 ? '' : 's'}`}</span></summary>
          <div className="mt-3"><div className="mb-3 flex justify-end"><button type="button" onClick={validate} className="text-[10px] font-bold uppercase text-[#00ff9c] hover:text-white">Run checks</button></div>{validationIssues.length === 0 ? <div className="flex items-center gap-2 text-[10px] text-[#72e6b4]"><CheckCircle2 className="h-3.5 w-3.5" />No issues detected</div> : <div className="space-y-2">{validationIssues.slice(0, 6).map(issue => <div key={issue.id} className={`flex gap-2 text-[10px] ${issue.severity === 'error' ? 'text-red-300' : 'text-amber-300'}`}><AlertTriangle className="mt-0.5 h-3 w-3 shrink-0" /><span><strong>{issue.title}:</strong> {issue.detail}</span></div>)}{validationIssues.length > 6 && <p className="text-[9px] text-zinc-600">+{validationIssues.length - 6} more issues</p>}</div>}</div>
        </details>
